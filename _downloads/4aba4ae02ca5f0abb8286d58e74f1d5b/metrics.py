...
class KLMetric(BaseMetric):
    def __init__(self, device='cuda', config=None):
        ...
        self.add_state("kl_sum", default=[], dist_reduce_fx=None)
    def update(self, output, batch):
        self.kl_sum.append(output.score*self.config.ml.model.FlipyFlopy.klw)
    def compute(self):
        return torch.tensor(self.kl_sum).mean()
