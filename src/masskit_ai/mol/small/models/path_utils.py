...
def get_value_attr(some_thing, some_key):
...
def get_path_input(mols, shortest_paths, max_atoms, args, output_tensor=True):
    ...
    if output_tensor:
        path_input = torch.tensor(path_input, device=device)
        path_input = path_input.view(
            [batch_size, max_atoms, max_atoms, n_path_features])
        path_mask = torch.tensor(path_mask, device=device)
...
def merge_path_inputs(path_inputs, path_masks, max_atoms, args):
    ...
    batch_size, max_path_length = len(path_inputs), get_value_attr(args, 'max_path_length')
    ...
    padded_path_inputs = torch.zeros(feature_shape, device=device)
    padded_mask = torch.zeros(mask_shape, device=device)
    ...
    for idx, path_input in enumerate(path_inputs):
        ...
        path_mask = torch.tensor(path_masks[idx], device=device)
...
def get_num_path_features(args):
    ...
    """
    num_features = 0
    num_features = get_value_attr(args, 'max_path_length') * mol_features.N_BOND_FEATS
    if get_value_attr(args, 'p_embed'):
        num_features += get_value_attr(args, 'max_path_length') + 2
