...
from masskit.utils.general import MassKitSearchPathPlugin, class_for_name, parse_filename
...
def aims_app(config: DictConfig) -> None:
    ...
    if query_file == search_file:
        ...
        if input_file_extension == 'msp':
            query_map = ArrowLibraryMap(load_msp2array(
                query_file, num=config.input.query.num))
        elif input_file_extension == 'sdf':
            query_map = ArrowLibraryMap(load_sdf2array(
                query_file, num=config.input.query.num))
        elif input_file_extension == 'pkl':
            df = pd.read_pickle(query_file)
            if config.input.query.num:
                df = df[0:config.input.query.num]
            query_map = PandasLibraryMap(
                df, column_name=config.input.query.column_name)
        elif input_file_extension == 'mgf':
            query_map = ArrowLibraryMap(load_mgf2array(
                query_file, num=config.input.query.num))
    ...
    index = class_for_name(config.paths.modules.indices,
                           config.search.index_type.name)(config.input.index.name)
    ...
    id_list_query = None
    query_num = config.input.query.num if config.input.query.num else len(
        query_map)
    if config.input.query.ids:
        queries = [query_map.getitem_by_id(
            x)['spectrum'] for x in config.input.query.ids]
        ...
        queries = query_map.to_arrow().slice(0, query_num)[
            config.input.query.column_name].to_numpy()
        ...
        queries = [query_map[x]['spectrum']
                   for x in random.sample(range(len(query_map)), query_num)]
    ...
    start_time = timeit.default_timer()
    hitlist = index.search(queries, id_list=search_map.get_ids(
    ), id_list_query=id_list_query, hitlist_size=config.search.hitlist.size)
    elapsed = timeit.default_timer() - start_time
    ...
    if config.search.score.column_name == 'cosine_score':
        if "cosine_score" not in hitlist.to_pandas().columns:
            CosineScore(search_map, query_table_map=query_map,
                        score_name=config.search.score.column_name).score(hitlist)
    elapsed = timeit.default_timer() - start_time
