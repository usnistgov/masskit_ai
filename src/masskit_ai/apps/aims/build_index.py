...
from masskit.utils.general import class_for_name, MassKitSearchPathPlugin
...
def build_index_app(config: DictConfig) -> None:
    ...
    if config.creation.index_type.input_type == 'fingerprint':
        index.create_from_fingerprint(
            library_map, config.input.library.column_name, config.input.library.column_count_name)
    else:
